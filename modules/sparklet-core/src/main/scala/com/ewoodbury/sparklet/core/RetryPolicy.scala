...
object RetryPolicy {
   ...
   * @param retryableExceptions Set of exception classes that are considered retryable.
   *                           If empty, all exceptions are considered retryable.
   */
   ...
   */
  val NoRetry: RetryPolicy = ExponentialBackoff(0, FiniteDuration(1, "ms"), FiniteDuration(1, "ms"))
  ...
  def forExceptions(maxRetries: Int, baseDelay: FiniteDuration, maxDelay: FiniteDuration)
                   (exceptions: Class[_]*): RetryPolicy =
