...
/**
 * Service Provider Interface for executing plans.
 * This allows the API module to trigger execution without depending on the execution module.
 */
...
/**
 * Global registry for the execution service.
 * The execution module will register an implementation here.
 */
object ExecutionService {
  @SuppressWarnings(Array("org.wartremover.warts.Var"))
  @volatile private var current: ExecutionService = new ExecutionService {
    def execute[A](plan: Plan[A]): Seq[A] = {
      throw new UnsupportedOperationException("No execution service registered. Please ensure the execution module is properly initialized.")
    }
