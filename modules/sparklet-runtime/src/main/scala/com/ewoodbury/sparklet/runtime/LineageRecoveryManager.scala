...
/**
 * Manages recovery of failed tasks through lineage-based recomputation. Features exponential
 * backoff and enhanced statistics tracking.
 *
 * TODO: Add shuffle service to the constructor to allow for reading shuffle data when we're ready
 * to support recovery of complex operations (joins, aggregations, etc.)
 *
 * @param taskReconstructor
 *   The task reconstructor to use for reconstructing tasks
 * @param maxRecoveryAttempts
 *   The maximum number of recovery attempts to make
 * @param baseBackoffDelay
...
class LineageRecoveryManager[F[_]: Async](
  ...
  /**
   * Attempt to recover a failed task using its lineage information. Includes exponential backoff
   * between attempts.
   *
   * @param taskId
   ...
   * @return
   *   Effect that yields Some(partition) if recovery succeeded, None otherwise
   */
  def recoverFailedTask(
    ...
    (for {
      ...
      _ = logger.info(
        s"Attempting recovery for task $taskId (attempt ${entry.attemptCount + 1}/$maxRecoveryAttempts)",
      )
      ...
      result <- reconstructedTaskOpt match {
          ...
          executeReconstructedTask(reconstructedTask, lineage).attempt.flatMap {
            case Right(partition) =>
              ...
              updateStats(_.recordSuccess(duration)) *>
                logger
                  .info(
                    s"Successfully recovered task with operation: ${lineage.operation} in ${duration.toMillis}ms",
                  )
                  .pure[F] *>
            ...
            case Left(recoveryException) =>
              ...
              updateStats(_.recordFailure(duration)) *>
                logger
                  .warn(
                    s"Recovery failed for operation: ${lineage.operation} after ${duration.toMillis}ms. Original: ${originalException.getMessage}, Recovery: ${recoveryException.getMessage}",
                  )
                  .pure[F] *>
                None.pure[F]
          }
        case None =>
          updateStats(_.incrementReconstructionFailures) *>
            logger
              .warn(s"Could not reconstruct task for operation: ${lineage.operation}")
              .pure[F] *>
            None.pure[F]
      }
    } yield result
   ...
   *   Task to execute
   * @param lineage
   *   Lineage information for the task
   * @return
   *   Effect that yields Partition[Any]
   */
  ...
  /**
   * Calculate exponential backoff delay based on attempt number. Formula: baseDelay *
   * 2^(attempt-1), capped at 30 seconds
   */
