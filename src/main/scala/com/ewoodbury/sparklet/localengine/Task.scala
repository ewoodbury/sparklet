...
object Task:
  ...
  case class FilterKeysTask[K, V](
    ...
    override def run(): Partition[(K, V)] = {
      println(s"[Thread: ${Thread.currentThread().getName}] Running FilterKeysTask on partition...")
  ...
  case class FilterValuesTask[K, V](
    ...
    override def run(): Partition[(K, V)] = {
      println(s"[Thread: ${Thread.currentThread().getName}] Running FilterValuesTask on partition...")
