...
trait TaskScheduler[F[_]]:
   ...
   *
   * @param tasks
   *   The tasks to execute. All tasks are assumed to be independent and can be executed in parallel
   *   with bounded concurrency depending on the scheduler implementation.
   * @tparam A
   *   Input element type for each task's partition.
   * @tparam B
   *   Output element type produced by each task.
   * @return
   *   An effect that yields the sequence of output partitions in the same order as the input tasks.
   */
  def submit[A, B](tasks: Seq[Task[A, B]]): F[Seq[Partition[B]]]
