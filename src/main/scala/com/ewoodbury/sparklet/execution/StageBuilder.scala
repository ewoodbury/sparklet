 ...
 * and shuffle boundaries for wide transformations.
 */
object StageBuilder:
  ...
  private def buildStagesRecursive(
    ...
    plan match {
      ...
      case Plan.MapOp(sourcePlan, f) =>
        ...
        extendStageOrCreateNew(sourceStageId, Stage.map(f).asInstanceOf[Stage[Any, Any]], stageMap, dependencies)

      case Plan.FilterOp(sourcePlan, p) =>
        val sourceStageId = buildStagesRecursive(sourcePlan, stageMap, dependencies)
        extendStageOrCreateNew(sourceStageId, Stage.filter(p).asInstanceOf[Stage[Any, Any]], stageMap, dependencies)

      case Plan.FlatMapOp(sourcePlan, f) =>
        val sourceStageId = buildStagesRecursive(sourcePlan, stageMap, dependencies)
        extendStageOrCreateNew(sourceStageId, Stage.flatMap(f).asInstanceOf[Stage[Any, Any]], stageMap, dependencies)

      case Plan.DistinctOp(sourcePlan) =>
        val sourceStageId = buildStagesRecursive(sourcePlan, stageMap, dependencies)
        extendStageOrCreateNew(sourceStageId, Stage.distinct.asInstanceOf[Stage[Any, Any]], stageMap, dependencies)

      case Plan.KeysOp(sourcePlan) =>
        val sourceStageId = buildStagesRecursive(sourcePlan, stageMap, dependencies)
        extendStageOrCreateNew(sourceStageId, Stage.keys.asInstanceOf[Stage[Any, Any]], stageMap, dependencies)

      case Plan.ValuesOp(sourcePlan) =>
        val sourceStageId = buildStagesRecursive(sourcePlan, stageMap, dependencies)
        extendStageOrCreateNew(sourceStageId, Stage.values.asInstanceOf[Stage[Any, Any]], stageMap, dependencies)

      case Plan.MapValuesOp(sourcePlan, f) =>
        val sourceStageId = buildStagesRecursive(sourcePlan, stageMap, dependencies)
        extendStageOrCreateNew(sourceStageId, Stage.mapValues(f).asInstanceOf[Stage[Any, Any]], stageMap, dependencies)

      case Plan.FilterKeysOp(sourcePlan, p) =>
        val sourceStageId = buildStagesRecursive(sourcePlan, stageMap, dependencies)
        extendStageOrCreateNew(sourceStageId, Stage.filterKeys(p).asInstanceOf[Stage[Any, Any]], stageMap, dependencies)

      case Plan.FilterValuesOp(sourcePlan, p) =>
        val sourceStageId = buildStagesRecursive(sourcePlan, stageMap, dependencies)
        extendStageOrCreateNew(sourceStageId, Stage.filterValues(p).asInstanceOf[Stage[Any, Any]], stageMap, dependencies)

      case Plan.FlatMapValuesOp(sourcePlan, f) =>
        val sourceStageId = buildStagesRecursive(sourcePlan, stageMap, dependencies)
        extendStageOrCreateNew(sourceStageId, Stage.flatMapValues(f).asInstanceOf[Stage[Any, Any]], stageMap, dependencies)
      ...
      case groupByKey: Plan.GroupByKeyOp[_, _] =>
        ...
        createShuffleStage(sourceStageId, "groupByKey", stageMap, dependencies, None, Some(groupByKey))

      case reduceByKey: Plan.ReduceByKeyOp[_, _] =>
        val sourceStageId = buildStagesRecursive(reduceByKey.source, stageMap, dependencies)
        createShuffleStage(sourceStageId, "reduceByKey", stageMap, dependencies, Some(reduceByKey.reduceFunc), Some(reduceByKey))

      case sortBy: Plan.SortByOp[_, _] =>
        val sourceStageId = buildStagesRecursive(sortBy.source, stageMap, dependencies)
        createShuffleStage(sourceStageId, "sortBy", stageMap, dependencies, Some(sortBy.keyFunc), Some(sortBy))
  ...
  /**
   * Extends an existing stage with a new narrow transformation, or creates a new stage
   * if the existing stage is a shuffle stage.
   */
  ...
  private def createShuffleStage(
    ...
    val shuffleStage = Stage.SingleOpStage[Any, Any](identity) // Will be replaced with actual shuffle logic
    
    // Set up shuffle input source - the shuffle stage reads from the shuffle data produced by the source stage
  ...
  /**
   * Legacy method for backward compatibility with current Executor.
   * Will be removed once DAGScheduler is fully integrated.
   */
  ...
  private def containsShuffleOperations(plan: Plan[_]): Boolean = plan match {
    ...
    case Plan.UnionOp(left, right) => containsShuffleOperations(left) || containsShuffleOperations(right)
    case _ => false
  }
