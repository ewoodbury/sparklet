...
/**
 * Represents the logical plan for computing a DistCollection. This is a Directed Acyclic Graph
 * (DAG) where nodes are operations. It's defined recursively.
 *
 * @tparam A
...
object Plan:
  /**
   * The starting point: represents the initial data source. For this local example, it holds an
   * Iterable directly. Using a function `() => Iterable[A]` makes it slightly lazier, ensuring the
   * source data isn't iterated until execution starts.
   */
